...
function addGamesToPage(games) {
    ...
    for( let i = 0; i < games.length; i++ ){
        ...
        newDiv.innerHTML = `<ul> <img class="game-img" src=${game.img}> </img> ${game.name}, ${game.description} </ul>`;

        // append the game to the games-container
        let element = document.getElementById("games-container");
...
let fundedTotal = funded.reduce( (acc,game)=>{
    return acc + game.pledged;
},0);
...
let fundedPlural = plural(funded.length);
let unfundedPlural = plural(unfunded.length); 

// create a string that explains the number of unfunded games using the ternary operator
let str = `A total of $${fundedTotal.toLocaleString('en-US')} has been raised for ${funded.length} game${fundedPlural}. `;
str += `Currently, ${unfunded.length} game${unfundedPlural} remain unfunded. We need your help to fund these amazing games!`;
